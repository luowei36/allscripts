...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for(let n = 0; n < cookiesArr.length; n++){
    	...
    	$.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
    	UA = UAInfo[$.UserName]
    	console.log(`【${$.UserName}】红包信息`)
    	await getRedInfo();
    	RemainInvites[n] = $.RemainInvite;
    }
    $.UserName = UserNameBak;
    cookie = cookieBak;
    UA=UABak				   
	for (let j = 0; j < $.packetIdArr.length && $.canHelp; j++) {
      ...
      if ($.UserName === $.packetIdArr[j].userName) {
		...
		console.log(`【${$.packetIdArr[j].userName}】需邀人数${RemainInvites[j]} 剩余${cookiesArr.length-i}人`)
      if(RemainInvites[j] > cookiesArr.length-i)
      {
      	console.log(`${RemainInvites[j]} 剩余助力次数不满足开红包，跳过`)
        continue;
      }
      $.max = false;
...
function getRedInfo(){
    ...
    $.get(options, (err, resp, data) => {
      try {
        ...
        //console.log('获取红包信息', data)
        data = JSON.parse(data)
        if (data.iRet === 0) {
        	console.log(`当前在开${data.Data.dwCurrentGrade}号红包，已有${data.Data.dwHelpedTimes}人助力，还需${data.Data.dwRemainInvite}人开下一个红包`)
			  $.RemainInvite = data.Data.dwRemainInvite;
			  }
			}
		} catch (e) {
        $.logErr(e, resp);
